...
const track_section_t small_flat_to_steep_diag={"small_flat_to_steep_diag",TRACK_DIAGONAL,small_flat_to_steep_diag_curve,SMALL_FLAT_TO_STEEP_DIAG_LENGTH};
const track_section_t small_steep_to_flat_diag={"small_steep_to_flat_diag",TRACK_DIAGONAL,small_steep_to_flat_diag_curve,SMALL_FLAT_TO_STEEP_DIAG_LENGTH};
const track_section_t flat_to_steep_diag={"flat_to_steep_diag",TRACK_DIAGONAL,flat_to_steep_diag_curve,FLAT_TO_STEEP_DIAG_LENGTH};
const track_section_t steep_to_flat_diag={"steep_to_flat_diag",TRACK_DIAGONAL,steep_to_flat_diag_curve,FLAT_TO_STEEP_DIAG_LENGTH};
const track_section_t steep_to_vertical_diag={"steep_to_vertical_diag",TRACK_DIAGONAL,steep_to_vertical_diag_curve,STEEP_TO_VERTICAL_DIAG_LENGTH};
...
const track_section_t gentle_to_left_bank_diag={"gentle_to_left_bank_diag",TRACK_DIAGONAL|TRACK_EXIT_BANK_LEFT|TRACK_SUPPORT_BASE,gentle_to_left_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t gentle_to_right_bank_diag={"gentle_to_right_bank_diag",TRACK_DIAGONAL|TRACK_EXIT_BANK_RIGHT|TRACK_SUPPORT_BASE,gentle_to_right_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
...
#define SMALL_TURN_STEEP_LENGTH (4.027196*TILE_SIZE)
#define LARGE_TURN_STEEP_LENGTH (4.930971*TILE_SIZE)
...
track_point_t small_turn_left_steep_curve(float distance)
{
	return sloped_turn_left_curve(1.5*TILE_SIZE,16*CLEARANCE_HEIGHT/SMALL_TURN_LENGTH,distance);
}
track_point_t small_turn_right_steep_curve(float distance)
{
	return sloped_turn_right_curve(1.5*TILE_SIZE,16*CLEARANCE_HEIGHT/SMALL_TURN_LENGTH,distance);
}
track_point_t large_turn_left_to_diag_steep_curve(float distance)
{
	float u=reparameterize(7.91458958e-10,-3.35963765e-08,6.19502002e-07,-5.22008490e-06,3.90190055e-05,5.61490975e-05,5.29026085e-02,distance);

	track_point_t point;
	point.position=vector3(cubic(8*CLEARANCE_HEIGHT-2*TILE_SIZE,3*TILE_SIZE-8*CLEARANCE_HEIGHT,0,0,u),cubic(-2.985488*CLEARANCE_HEIGHT,-0.965078*CLEARANCE_HEIGHT,23.950566*CLEARANCE_HEIGHT,0,u),cubic(68*CLEARANCE_HEIGHT/3-5*TILE_SIZE,7.5*TILE_SIZE-112*CLEARANCE_HEIGHT/3,44*CLEARANCE_HEIGHT/3,0,u));
	point.tangent=vector3_normalize(vector3(cubic_derivative(8*CLEARANCE_HEIGHT-2*TILE_SIZE,3*TILE_SIZE-8*CLEARANCE_HEIGHT,0,u),cubic_derivative(-2.985488*CLEARANCE_HEIGHT,-0.965078*CLEARANCE_HEIGHT,23.950566*CLEARANCE_HEIGHT,u),cubic_derivative(68*CLEARANCE_HEIGHT/3-5*TILE_SIZE,7.5*TILE_SIZE-112*CLEARANCE_HEIGHT/3,44*CLEARANCE_HEIGHT/3,u)));
	point.normal=vector3(0.0,1.0,0.0);
...
track_point_t large_turn_right_to_diag_steep_curve(float distance)
{
	track_point_t point=large_turn_left_to_diag_steep_curve(distance);
	point.position.x*=-1;
	point.normal.x*=-1;
	point.tangent.x*=-1;
	point.binormal.y*=-1;
...
track_point_t large_turn_left_to_orthogonal_steep_curve(float distance)
{
	float u=reparameterize(7.91458971e-10,-5.65550966e-08,1.74026892e-06,-3.10997900e-05,3.86655610e-04,-3.98950137e-03,8.58061736e-02,distance);

	track_point_t point;
	point.position=vector3(cubic(-24*CLEARANCE_HEIGHT/3+2*TILE_SIZE,-3*TILE_SIZE+48*CLEARANCE_HEIGHT/3,-8*CLEARANCE_HEIGHT,0,u),cubic(-2.985488*CLEARANCE_HEIGHT,9.921543*CLEARANCE_HEIGHT,13.063945*CLEARANCE_HEIGHT,0,u),cubic(68*CLEARANCE_HEIGHT/3-5*TILE_SIZE,7.5*TILE_SIZE-92*CLEARANCE_HEIGHT/3,8*CLEARANCE_HEIGHT,0,u));
	point.tangent=vector3_normalize(vector3(cubic_derivative(-24*CLEARANCE_HEIGHT/3+2*TILE_SIZE,-3*TILE_SIZE+48*CLEARANCE_HEIGHT/3,-8*CLEARANCE_HEIGHT,u),cubic_derivative(-2.985488*CLEARANCE_HEIGHT,9.921543*CLEARANCE_HEIGHT,13.063945*CLEARANCE_HEIGHT,u),cubic_derivative(68*CLEARANCE_HEIGHT/3-5*TILE_SIZE,7.5*TILE_SIZE-92*CLEARANCE_HEIGHT/3,8*CLEARANCE_HEIGHT,u)));
	point.normal=vector3(0.0,1.0,0.0);
	point.binormal=vector3_normalize(vector3_cross(point.tangent,point.normal));
	point.normal=vector3_normalize(vector3_cross(point.binormal,point.tangent));
	return point;
}
track_point_t large_turn_right_to_orthogonal_steep_curve(float distance)
{
	track_point_t point=large_turn_left_to_orthogonal_steep_curve(distance);
...
const track_section_t medium_turn_left_gentle={"medium_turn_left_%d %d gentle",TRACK_OFFSET_SPRITE_MASK|TRACK_SUPPORT_BASE|TRACK_EXIT_90_DEG_LEFT,medium_turn_left_gentle_curve,MEDIUM_TURN_GENTLE_LENGTH};
...
const track_section_t large_turn_left_to_diag_gentle={"large_turn_left_to_diag_gentle",TRACK_EXIT_45_DEG_LEFT,large_turn_left_to_diag_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
const track_section_t large_turn_right_to_diag_gentle={"large_turn_right_to_diag_gentle",TRACK_EXIT_45_DEG_RIGHT,large_turn_right_to_diag_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
const track_section_t large_turn_left_to_orthogonal_gentle={"large_turn_left_to_orthogonal_gentle",TRACK_DIAGONAL_2|TRACK_EXIT_45_DEG_LEFT,large_turn_left_to_orthogonal_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
const track_section_t large_turn_right_to_orthogonal_gentle={"large_turn_right_to_orthogonal_gentle",TRACK_DIAGONAL_2|TRACK_EXIT_45_DEG_RIGHT,large_turn_right_to_orthogonal_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
...
const track_section_t small_turn_left_steep={"small_turn_left_steep",TRACK_OFFSET_SPRITE_MASK|TRACK_SUPPORT_BASE|TRACK_EXIT_90_DEG_LEFT|TRACK_ALT_INVERT,small_turn_left_steep_curve,SMALL_TURN_STEEP_LENGTH};
const track_section_t small_turn_right_steep={"small_turn_right_steep",TRACK_OFFSET_SPRITE_MASK|TRACK_SUPPORT_BASE|TRACK_EXIT_90_DEG_RIGHT|TRACK_ALT_INVERT,small_turn_right_steep_curve,SMALL_TURN_STEEP_LENGTH};
const track_section_t large_turn_left_to_diag_steep={"large_turn_left_to_diag_steep",TRACK_EXIT_45_DEG_LEFT|TRACK_ALT_INVERT,large_turn_left_to_diag_steep_curve,LARGE_TURN_STEEP_LENGTH};
const track_section_t large_turn_right_to_diag_steep={"large_turn_right_to_diag_steep",TRACK_EXIT_45_DEG_RIGHT|TRACK_ALT_INVERT,large_turn_right_to_diag_steep_curve,LARGE_TURN_STEEP_LENGTH};
const track_section_t large_turn_left_to_orthogonal_steep={"large_turn_left_to_orthogonal_steep",TRACK_DIAGONAL_2|TRACK_EXIT_45_DEG_LEFT|TRACK_ALT_INVERT,large_turn_left_to_orthogonal_steep_curve,LARGE_TURN_STEEP_LENGTH};
const track_section_t large_turn_right_to_orthogonal_steep={"large_turn_right_to_orthogonal_steep",TRACK_DIAGONAL_2|TRACK_EXIT_45_DEG_RIGHT|TRACK_ALT_INVERT,large_turn_right_to_orthogonal_steep_curve,LARGE_TURN_STEEP_LENGTH};
...
const track_section_t gentle_to_gentle_left_bank_diag={"gentle_to_gentle_left_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_EXIT_BANK_LEFT,gentle_to_gentle_left_bank_diag_curve,GENTLE_DIAG_LENGTH};
const track_section_t gentle_to_gentle_right_bank_diag={"gentle_to_gentle_right_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_EXIT_BANK_RIGHT,gentle_to_gentle_right_bank_diag_curve,GENTLE_DIAG_LENGTH};
const track_section_t gentle_left_bank_to_gentle_diag={"gentle_left_bank_to_gentle_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_ENTRY_BANK_LEFT,gentle_left_bank_to_gentle_diag_curve,GENTLE_DIAG_LENGTH};
const track_section_t gentle_right_bank_to_gentle_diag={"gentle_right_bank_to_gentle_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_ENTRY_BANK_RIGHT,gentle_right_bank_to_gentle_diag_curve,GENTLE_DIAG_LENGTH};
const track_section_t left_bank_to_gentle_left_bank_diag={"left_bank_to_gentle_left_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_BANK_LEFT,left_bank_to_gentle_left_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t right_bank_to_gentle_right_bank_diag={"right_bank_to_gentle_right_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_BANK_RIGHT,right_bank_to_gentle_right_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t gentle_left_bank_to_left_bank_diag={"gentle_left_bank_to_left_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_BANK_LEFT,gentle_left_bank_to_left_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t gentle_right_bank_to_right_bank_diag={"gentle_right_bank_to_right_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_BANK_RIGHT,gentle_right_bank_to_right_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t gentle_left_bank_diag={"gentle_left_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_BANK_LEFT,gentle_left_bank_diag_curve,GENTLE_DIAG_LENGTH};
const track_section_t gentle_right_bank_diag={"gentle_right_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_BANK_RIGHT,gentle_right_bank_diag_curve,GENTLE_DIAG_LENGTH};
const track_section_t flat_to_gentle_left_bank_diag={"flat_to_gentle_left_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_EXIT_BANK_LEFT,flat_to_gentle_left_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t flat_to_gentle_right_bank_diag={"flat_to_gentle_right_bank_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_EXIT_BANK_RIGHT,flat_to_gentle_right_bank_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t gentle_left_bank_to_flat_diag={"gentle_left_bank_to_flat_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_ENTRY_BANK_LEFT,gentle_left_bank_to_flat_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t gentle_right_bank_to_flat_diag={"gentle_right_bank_to_flat_diag",TRACK_DIAGONAL|TRACK_SUPPORT_BASE|TRACK_ENTRY_BANK_RIGHT,gentle_right_bank_to_flat_diag_curve,FLAT_TO_GENTLE_DIAG_LENGTH};
const track_section_t large_turn_left_bank_to_diag_gentle={"large_turn_left_bank_to_diag_gentle",TRACK_EXIT_90_DEG_LEFT|TRACK_EXIT_45_DEG_LEFT|TRACK_BANK_LEFT,large_turn_left_bank_to_diag_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
const track_section_t large_turn_right_bank_to_diag_gentle={"large_turn_right_bank_to_diag_gentle",TRACK_EXIT_45_DEG_RIGHT|TRACK_BANK_RIGHT,large_turn_right_bank_to_diag_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
const track_section_t large_turn_left_bank_to_orthogonal_gentle={"large_turn_left_bank_to_orthogonal_gentle",TRACK_DIAGONAL_2|TRACK_EXIT_45_DEG_LEFT|TRACK_BANK_LEFT,large_turn_left_bank_to_orthogonal_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
const track_section_t large_turn_right_bank_to_orthogonal_gentle={"large_turn_right_bank_to_orthogonal_gentle",TRACK_DIAGONAL_2|TRACK_EXIT_45_DEG_RIGHT|TRACK_BANK_RIGHT,large_turn_right_bank_to_orthogonal_gentle_curve,LARGE_TURN_GENTLE_LENGTH};
...
track_point_t steep_to_gentle_left_bank_curve(float distance)
{
	return banked_curve(steep_to_gentle_curve(distance),BANK_ANGLE*distance/GENTLE_TO_STEEP_LENGTH);
}
track_point_t steep_to_gentle_right_bank_curve(float distance)
{
	return banked_curve(steep_to_gentle_curve(distance),-BANK_ANGLE*distance/GENTLE_TO_STEEP_LENGTH);
}
track_point_t gentle_left_bank_to_steep_curve(float distance)
{
	return banked_curve(gentle_to_steep_curve(distance),BANK_ANGLE*(1.0-distance/GENTLE_TO_STEEP_LENGTH));
}
track_point_t gentle_right_bank_to_steep_curve(float distance)
{
	return banked_curve(gentle_to_steep_curve(distance),-BANK_ANGLE*(1.0-distance/GENTLE_TO_STEEP_LENGTH));
}

track_point_t steep_to_gentle_left_bank_diag_curve(float distance)
{
	return banked_curve(steep_to_gentle_diag_curve(distance),BANK_ANGLE*distance/GENTLE_TO_STEEP_DIAG_LENGTH);
}
track_point_t steep_to_gentle_right_bank_diag_curve(float distance)
{
	return banked_curve(steep_to_gentle_diag_curve(distance),-BANK_ANGLE*distance/GENTLE_TO_STEEP_DIAG_LENGTH);
}
track_point_t gentle_left_bank_to_steep_diag_curve(float distance)
{
	return banked_curve(gentle_to_steep_diag_curve(distance),BANK_ANGLE*(1.0-distance/GENTLE_TO_STEEP_DIAG_LENGTH));
}
track_point_t gentle_right_bank_to_steep_diag_curve(float distance)
{
	return banked_curve(gentle_to_steep_diag_curve(distance),-BANK_ANGLE*(1.0-distance/GENTLE_TO_STEEP_DIAG_LENGTH));
}
...
const track_section_t s_bend_left_bank={"s_bend_left_bank",TRACK_ENTRY_BANK_LEFT|TRACK_EXIT_BANK_RIGHT,s_bend_left_bank_curve,S_BEND_LENGTH};
const track_section_t s_bend_right_bank={"s_bend_right_bank",TRACK_ENTRY_BANK_RIGHT|TRACK_EXIT_BANK_LEFT,s_bend_right_bank_curve,S_BEND_LENGTH};
...
const track_section_t gentle_left_bank_to_steep={"gentle_left_bank_to_steep",TRACK_ENTRY_BANK_LEFT|TRACK_ALT_PREFER_ODD,gentle_left_bank_to_steep_curve,GENTLE_TO_STEEP_LENGTH};
const track_section_t gentle_right_bank_to_steep={"gentle_right_bank_to_steep",TRACK_ENTRY_BANK_RIGHT|TRACK_ALT_PREFER_ODD,gentle_right_bank_to_steep_curve,GENTLE_TO_STEEP_LENGTH};
const track_section_t steep_to_gentle_left_bank={"steep_to_gentle_left_bank",TRACK_EXIT_BANK_LEFT|TRACK_ALT_INVERT|TRACK_ALT_PREFER_ODD,steep_to_gentle_left_bank_curve,GENTLE_TO_STEEP_LENGTH};
const track_section_t steep_to_gentle_right_bank={"steep_to_gentle_right_bank",TRACK_EXIT_BANK_RIGHT|TRACK_ALT_INVERT|TRACK_ALT_PREFER_ODD,steep_to_gentle_right_bank_curve,GENTLE_TO_STEEP_LENGTH};

const track_section_t gentle_left_bank_to_steep_diag={"gentle_left_bank_to_steep_diag",TRACK_DIAGONAL|TRACK_ENTRY_BANK_LEFT|TRACK_SUPPORT_BASE,gentle_left_bank_to_steep_diag_curve,GENTLE_TO_STEEP_DIAG_LENGTH};
const track_section_t gentle_right_bank_to_steep_diag={"gentle_right_bank_to_steep_diag",TRACK_DIAGONAL|TRACK_ENTRY_BANK_RIGHT|TRACK_SUPPORT_BASE,gentle_right_bank_to_steep_diag_curve,GENTLE_TO_STEEP_DIAG_LENGTH};
const track_section_t steep_to_gentle_left_bank_diag={"steep_to_gentle_left_bank_diag",TRACK_DIAGONAL|TRACK_EXIT_BANK_LEFT|TRACK_SUPPORT_BASE,steep_to_gentle_left_bank_diag_curve,GENTLE_TO_STEEP_DIAG_LENGTH};
const track_section_t steep_to_gentle_right_bank_diag={"steep_to_gentle_right_bank_diag",TRACK_DIAGONAL|TRACK_EXIT_BANK_RIGHT|TRACK_SUPPORT_BASE,steep_to_gentle_right_bank_diag_curve,GENTLE_TO_STEEP_DIAG_LENGTH};
...
#define CORKSCREW_DIAG_LENGTH 2*2.008248*TILE_SIZE
#define LARGE_CORKSCREW_DIAG_LENGTH 2*2.821776*TILE_SIZE
...
track_point_t corkscrew_left_diag_curve(float distance)
{
	if(distance<CORKSCREW_DIAG_LENGTH/2)
	{
	return bezier3d(1.18913994e-01,2.56887901e+00,-5.49450000e+00,0.00000000e+00,-1.93778728e+00,4.74449427e+00,0.00000000e+00,0.00000000e+00,-3.21470313e-01,-2.23029687e-01,5.49450000e+00,0.00000000e+00,1.81753313e-01,-9.57262309e-01,1.11180103e+00,-3.36335475e-01,7.34632534e-08,-1.64549304e-06,1.82893882e-05,-6.18083990e-05,-1.80249700e-04,4.16890073e-03,1.28703651e-01,distance);
	}
	else
	{
	return bezier3d(-1.18913994e-01,2.92562099e+00,0.00000000e+00,-2.80670700e+00,-1.93778728e+00,1.06886756e+00,3.67562672e+00,2.80670700e+00,-3.21470313e-01,1.18744063e+00,4.08402969e+00,4.95000000e+00,1.81753313e-01,4.12002371e-01,-2.57463652e-01,4.34425627e-05,7.34632528e-08,-1.76250597e-06,2.06157991e-05,-2.08576046e-04,1.59477935e-03,-9.64216613e-03,1.82000876e-01,distance-CORKSCREW_DIAG_LENGTH/2);
	}
}
track_point_t corkscrew_right_diag_curve(float distance)
{
	track_point_t point=corkscrew_left_diag_curve(distance);
...
track_point_t large_corkscrew_left_diag_curve(float distance)
{
	if(distance<LARGE_CORKSCREW_DIAG_LENGTH/2)
	{
	return bezier3d(4.83380995e-01,3.23492851e+00,-7.92000000e+00,0.00000000e+00,-2.35754915e+00,6.51147550e+00,0.00000000e+00,0.00000000e+00,6.70303564e-01,-1.99030356e+00,7.92000000e+00,0.00000000e+00,4.37296633e-01,-1.23490126e+00,8.44367318e-01,-1.34344610e-01,-4.99276728e-09,7.70966321e-08,6.39351324e-07,-1.64550789e-05,1.98728029e-06,2.60557647e-03,8.92914361e-02,distance);

	}
	else
	{
	return bezier3d(-4.83380995e-01,4.68507149e+00,0.00000000e+00,-4.20169050e+00,-2.35754915e+00,5.61171939e-01,5.95030356e+00,4.15392636e+00,6.70303564e-01,-2.06071274e-02,5.95030356e+00,6.60000000e+00,4.37296633e-01,-7.69886414e-02,-3.13545298e-01,8.75819169e-02,-4.99276733e-09,2.48347035e-07,-4.14462823e-06,2.75080451e-05,-1.25398953e-04,-6.16733655e-04,1.18825089e-01,distance-LARGE_CORKSCREW_DIAG_LENGTH/2);
	}
}
track_point_t large_corkscrew_right_diag_curve(float distance)
{
	track_point_t point=large_corkscrew_left_diag_curve(distance);
...
#define HALF_LOOP_DIAG_SEGMENT1_LENGTH 4.038412*TILE_SIZE
#define HALF_LOOP_DIAG_SEGMENT2_LENGTH 1.957225*TILE_SIZE
#define HALF_LOOP_DIAG_LENGTH (HALF_LOOP_DIAG_SEGMENT1_LENGTH+HALF_LOOP_DIAG_SEGMENT2_LENGTH)

track_point_t half_loop_diag_curve(float distance)
{
	if(distance<HALF_LOOP_DIAG_SEGMENT1_LENGTH)
		return cubic_curve_vertical_diagonal(-0.2*FLAT_DIAG_LENGTH,-1.45*FLAT_DIAG_LENGTH,3.5*FLAT_DIAG_LENGTH,0,CLEARANCE_HEIGHT,5*CLEARANCE_HEIGHT,7*CLEARANCE_HEIGHT,0,9.43043497e-09,-3.34492102e-07,3.75540523e-06,-1.89764219e-05,9.01595836e-05,1.08291183e-03,5.88296634e-02,distance);
	else
		return cubic_curve_vertical_diagonal(1.7*FLAT_DIAG_LENGTH-10*CLEARANCE_HEIGHT,-2.55*FLAT_DIAG_LENGTH+10*CLEARANCE_HEIGHT,0,1.85*FLAT_DIAG_LENGTH,-8*CLEARANCE_HEIGHT/3,-CLEARANCE_HEIGHT,10*CLEARANCE_HEIGHT,13*CLEARANCE_HEIGHT,2.12409372e-07,-4.29724755e-06,2.49160850e-05,-3.38488798e-05,-2.03217868e-04,2.08320996e-03,1.48489512e-01,distance-HALF_LOOP_DIAG_SEGMENT1_LENGTH);
}
#define VERTICAL_LOOP_DIAG_FACTOR 1.005375
#define VERTICAL_LOOP_DIAG_SEGMENT1_LENGTH 3.769977*TILE_SIZE
#define VERTICAL_LOOP_DIAG_PROJ_LENGTH (VERTICAL_LOOP_DIAG_SEGMENT1_LENGTH+1.957225*TILE_SIZE)
#define VERTICAL_LOOP_DIAG_LENGTH 5.741719*TILE_SIZE

track_point_t vertical_loop_left_diag_curve(float distance)
{
track_point_t point;
float u=reparameterize(-3.47203927e-16,-2.11725633e-15,1.04739711e-11,-2.98935812e-13,-3.76324884e-07,-2.56923623e-12,5.29107063e-02,distance);
float proj_distance=u*VERTICAL_LOOP_DIAG_PROJ_LENGTH ;
	if(proj_distance<VERTICAL_LOOP_DIAG_SEGMENT1_LENGTH)
		point=cubic_curve_vertical_diagonal(0.3*FLAT_DIAG_LENGTH,-2.2*FLAT_DIAG_LENGTH,3.5*FLAT_DIAG_LENGTH,0,-3*CLEARANCE_HEIGHT,9*CLEARANCE_HEIGHT,7*CLEARANCE_HEIGHT,0,-9.99885509e-10,9.63818926e-08,-2.38298043e-06,1.77517129e-05,-1.69267859e-05,1.77905279e-03,5.87532889e-02,proj_distance);
	else
		point=cubic_curve_vertical_diagonal(1.7*FLAT_DIAG_LENGTH-10*CLEARANCE_HEIGHT,-2.55*FLAT_DIAG_LENGTH+10*CLEARANCE_HEIGHT,0,1.6*FLAT_DIAG_LENGTH,-8*CLEARANCE_HEIGHT/3,-CLEARANCE_HEIGHT,10*CLEARANCE_HEIGHT,13*CLEARANCE_HEIGHT,2.12409372e-07,-4.29724755e-06,2.49160850e-05,-3.38488798e-05,-2.03217868e-04,2.08320996e-03,1.48489512e-01,proj_distance-VERTICAL_LOOP_DIAG_SEGMENT1_LENGTH);

float d=0.25*TILE_SIZE;
point.position.x+=d*u*u;
point.position.z+=d*u*u;
point.tangent.x+=d*2*u/VERTICAL_LOOP_DIAG_PROJ_LENGTH ;
point.tangent.z+=d*2*u/VERTICAL_LOOP_DIAG_PROJ_LENGTH ;

point.tangent=vector3_normalize(point.tangent);
point.normal=vector3_normalize(vector3_cross(point.tangent,vector3(1,0,1)));
point.binormal=vector3_cross(point.tangent,point.normal);
return point;
}


track_point_t vertical_loop_left_diag_curve_full(float distance)
{
	if(distance<VERTICAL_LOOP_DIAG_LENGTH/2)return vertical_loop_left_diag_curve(distance);

track_point_t point=vertical_loop_left_diag_curve((VERTICAL_LOOP_DIAG_LENGTH-distance));
...
point.position.x+=0.5*TILE_SIZE-1.5*TILE_SIZE;
point.position.z+=0.5*TILE_SIZE+1.5*TILE_SIZE;

point.binormal=vector3_cross(point.tangent,point.normal);
return point;
}

track_point_t vertical_loop_right_diag_curve(float distance)
{
	track_point_t point=vertical_loop_left_diag_curve(distance);
...
track_point_t banked_inline_twist_left_curve(float distance)
{
	return banked_curve(inline_twist_left_curve(distance),BANK_ANGLE*(1.0-distance/INLINE_TWIST_LENGTH));
}
track_point_t banked_inline_twist_right_curve(float distance)
{
	return banked_curve(inline_twist_right_curve(distance),-BANK_ANGLE*(1.0-distance/INLINE_TWIST_LENGTH));
}
track_point_t banked_barrel_roll_left_curve(float distance)
{
float u=distance/BARREL_ROLL_LENGTH;
float mid=2*BANK_ANGLE/M_PI;
float bank=0;
	if(u<mid)bank=(u-mid)*(u-mid)/(mid*mid);
return banked_curve(barrel_roll_left_curve(distance),BANK_ANGLE*bank);
}
track_point_t banked_barrel_roll_right_curve(float distance)
{
float u=distance/BARREL_ROLL_LENGTH;
float mid=2*BANK_ANGLE/M_PI;
float bank=0;
	if(u<mid)bank=(u-mid)*(u-mid)/(mid*mid);
return banked_curve(barrel_roll_right_curve(distance),-BANK_ANGLE*bank);
}
track_point_t banked_zero_g_roll_left_curve(float distance)
{
float u=distance/TILE_SIZE;
float bank=0;
	if(u<2)bank=1-u+0.25*u*u;
	return banked_curve(zero_g_roll_left_curve(distance),BANK_ANGLE*bank);
}
track_point_t banked_zero_g_roll_right_curve(float distance)
{
float u=distance/TILE_SIZE;
float bank=0;
	if(u<2)bank=1-u+0.25*u*u;
	return banked_curve(zero_g_roll_right_curve(distance),-BANK_ANGLE*bank);
}
...
const track_section_t inline_twist_left={"inline_twist_left",TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK,inline_twist_left_curve,INLINE_TWIST_LENGTH};
const track_section_t inline_twist_right={"inline_twist_right",TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK,inline_twist_right_curve,INLINE_TWIST_LENGTH};
...
const track_section_t dive_loop_45_left={"dive_loop_45_left",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_EXIT_45_DEG_LEFT,dive_loop_45_left_curve,DIVE_LOOP_45_LENGTH};
const track_section_t dive_loop_45_right={"dive_loop_45_right",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_EXIT_45_DEG_RIGHT,dive_loop_45_right_curve,DIVE_LOOP_45_LENGTH};
...
const track_section_t banked_inline_twist_left={"banked_inline_twist_left",TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK,banked_inline_twist_left_curve,INLINE_TWIST_LENGTH};
const track_section_t banked_inline_twist_right={"banked_inline_twist_right",TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK,banked_inline_twist_right_curve,INLINE_TWIST_LENGTH};
const track_section_t banked_barrel_roll_left={"banked_barrel_roll_left",TRACK_NO_SUPPORTS|TRACK_SPECIAL_BARREL_ROLL_LEFT|TRACK_OFFSET_SPRITE_MASK,banked_barrel_roll_left_curve,BARREL_ROLL_LENGTH};
const track_section_t banked_barrel_roll_right={"banked_barrel_roll_right",TRACK_NO_SUPPORTS|TRACK_SPECIAL_BARREL_ROLL_RIGHT|TRACK_OFFSET_SPRITE_MASK,banked_barrel_roll_right_curve,BARREL_ROLL_LENGTH};
...
const track_section_t corkscrew_left_diag={"corkscrew_left_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK|TRACK_EXIT_90_DEG_LEFT,corkscrew_left_diag_curve,CORKSCREW_DIAG_LENGTH};
const track_section_t corkscrew_right_diag={"corkscrew_right_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK|TRACK_EXIT_90_DEG_RIGHT,corkscrew_right_diag_curve,CORKSCREW_DIAG_LENGTH};


const track_section_t large_corkscrew_left_diag={"large_corkscrew_left_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK|TRACK_EXIT_90_DEG_LEFT,large_corkscrew_left_diag_curve,LARGE_CORKSCREW_DIAG_LENGTH};
const track_section_t large_corkscrew_right_diag={"large_corkscrew_right_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_OFFSET_SPRITE_MASK|TRACK_EXIT_90_DEG_RIGHT,large_corkscrew_right_diag_curve,LARGE_CORKSCREW_DIAG_LENGTH};

const track_section_t half_loop_diag={"half_loop_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_EXIT_180_DEG,half_loop_diag_curve,HALF_LOOP_DIAG_LENGTH};
const track_section_t vertical_loop_left_diag={"vertical_loop_left_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_EXIT_180_DEG,vertical_loop_left_diag_curve,VERTICAL_LOOP_DIAG_LENGTH};
const track_section_t vertical_loop_right_diag={"vertical_loop_right_diag",TRACK_DIAGONAL|TRACK_NO_SUPPORTS|TRACK_EXIT_180_DEG,vertical_loop_right_diag_curve,VERTICAL_LOOP_DIAG_LENGTH};
...
track_section_t track_sections[NUM_TRACK_SECTIONS]={
	...
	half_loop_diag,
	vertical_loop_left_diag,
	vertical_loop_right_diag
};
